...
void main() {
  ...
  group("$DocumentSnapshot", () {
    ...
    test('returns correct id', () async {
      DocumentSnapshot ds = await firestore.document('doc/exists').get();
      expect(ds.id, equals('exists'));
    });

    test('reference returns correct $DocumentReference', () async {
      DocumentReference expectedRef = firestore.document('doc/exists');
      DocumentSnapshot ds = await firestore.document('doc/exists').get();
    ...
    test('exists returns correct bool', () async {
      DocumentSnapshot ds = await firestore.document('doc/exists').get();
      DocumentSnapshot ds2 = await firestore.document('doc/not-exists').get();
    ...
    group('.get()', () {
      ...
      test('throws a [StateError] if document does not exist', () async {
        DocumentSnapshot ds = await firestore.document('doc/not-exists').get();
        try {
          ds.get('foo');
        } on StateError {
          return;
      ...
      test('gets a nested Map field by [String]', () async {
        DocumentSnapshot ds = await firestore.document('doc/get-test').get();
