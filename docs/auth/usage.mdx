...
To learn more about how you can handle any errors which are thrown from the method, view the [Error Handling](auth/error-handling)
documentation.
...
existing user accounts.  Users can then be identified using the Firebase UID, regardless of the provider they used to sign in.

The user must first be logged in to one of the accounts you want to link. You can then log the user into the second provider,
...
Future<void> linkGoogleandTwitter() async {
  ...
  // Store the UserCredential for later use
  ...
  // Link the Twitter account with the Google account
