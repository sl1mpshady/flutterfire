...
window to appear prompting the user to sign-in to your project. Alternatively you can use `signInWithRedirect` to keep the authentication
...
Ensure that the OAuth redirect URI from the Firebase console is added as a Valid OAuth Redirect URIs
...
as a callback URL in your Twitter application on the developer console.
...
You will need to create the GitHubSignIn instance with your GitHub ID and Secret, but also the Firebase callbackURL.
