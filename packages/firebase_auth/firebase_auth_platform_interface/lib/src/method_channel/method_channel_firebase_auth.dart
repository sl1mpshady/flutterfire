...
class MethodChannelFirebaseAuth extends FirebaseAuthPlatform {
  ...
  /// A generic handler for user based change listeners.
  Stream<UserPlatform> _addUserListener(String type) {
    int handle = MethodChannelFirebaseAuth.nextMethodChannelHandleId;

    StreamController<UserPlatform> controller;

    controller = StreamController<UserPlatform>.broadcast(onListen: () {
      MethodChannelFirebaseAuth.userObservers[handle] = controller;
      MethodChannelFirebaseAuth.channel.invokeMethod<void>(
        'Auth#$type',
        <String, dynamic>{
          'handle': handle,
          'appName': app.name,
      ...
      MethodChannelFirebaseAuth.userObservers.remove(handle);
      MethodChannelFirebaseAuth.channel.invokeMethod<void>(
        'Auth#removeListener',
        <String, dynamic>{
          'handle': handle,
  ...
  Stream<UserPlatform> authStateChanges() =>
      _addUserListener('authStateChanges');

  @override
  Stream<UserPlatform> idTokenChanges() => _addUserListener('idTokenChanges');
