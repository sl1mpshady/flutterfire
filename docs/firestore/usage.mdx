...
Firestore firestore = FirebaseFirestore.instanceFor(app: secondaryApp);
```
...
When perfoming a query, Firestore returns either a [`QuerySnapshot`](!cloud_firestore.QuerySnapshot) or a [`DocumentSnapshot`](!cloud_firestore.DocumentSnapshot).
