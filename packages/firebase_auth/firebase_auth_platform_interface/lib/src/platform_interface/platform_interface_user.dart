...
abstract class UserPlatform extends PlatformInterface {
  ...
  ///
  /// This is a read-only property, which indicates the tenant ID used to sign in the current user.
  ...
  ///  - Thrown if the user's last sign-in time does not meet the security threshold. Use [User.reauthenticateWithCredential] to resolve.
  ...
  /// - **invalid-credential**:
  ///  - Thrown if the provider's credential is not valid. This can happen if it has already expired when calling link, or if it used invalid token(s).
  ...
  ///    For example, this error could be thrown if you are upgrading an anonymous user to a Google user by linking a Google credential
  ...
  ///  - Thrown if the email corresponding to the credential already exists among your users. When thrown while linking a credential to an existing user,
  ///    an `email` and `credential` ([AuthCredential]) fields are also provided. You have to link the credential to the existing user with that email
  ///    if you wish to continue signing in with that credential. To do so, call [fetchSignInMethodsForEmail], sign in to `email` via one of the providers
  ///    returned and then [User.linkWithCredential] the original credential to that newly signed in user.
  ...
  ///
  /// An email will be sent to the original email address (if it was set) that allows to
  ...
  ///   If this requirement isn't met, ask the user to authenticate again and then call [User.reauthenticateWithCredential].
  ///
  /// A [FirebaseAuthException] maybe thrown with the following error code:
  /// - **invalid-email**:
